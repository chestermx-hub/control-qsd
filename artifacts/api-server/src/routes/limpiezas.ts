...
async function maybeCompleteExecution(executionId: number) {
  ...
  if (relevantActivities.every((activity) => (activity.completed || activity.notApplicable) && (!activity.requiresPhoto || (activity.initialPhoto && activity.finalPhoto))) && areas.every((area) => area.excluded || area.ready)) {
...
router.patch("/limpiezas/ejecuciones/:id/actividades/:activityId", async (req, res) => {
  ...
  if (nextCompleted && nextNotApplicable) { res.status(400).json({ error: "Una actividad no puede estar completada y marcada como no aplica" }); return; }
   if (nextCompleted && current.requiresPhoto && !nextInitial) { res.status(400).json({ error: "Esta actividad requiere una foto inicial antes de completarse" }); return; }
