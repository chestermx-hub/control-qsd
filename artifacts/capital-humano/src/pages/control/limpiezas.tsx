...
function PhotoButton({ label, value, onUploaded }: { label: string; value?: string; onUploaded: (path: string) => void }) {
  ...
  return <div><input ref={ref} type="file" accept="image/*" capture="environment" className="hidden" onChange={e => upload(e.target.files?.[0])} />{value ? <div className="space-y-2"><img src={value} className="h-28 w-full rounded-md object-cover border" /><Button type="button" variant="outline" size="sm" className="min-h-10" onClick={() => ref.current?.click()}><Upload className="mr-2 h-4 w-4" />Cambiar</Button></div> : <Button type="button" variant="outline" className="min-h-11 w-full" disabled={busy} onClick={() => ref.current?.click()}>{busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ImagePlus className="mr-2 h-4 w-4" />}{busy ? "Subiendo..." : label}</Button>}</div>;
}

function ActivityPhoto({ activity, onUploaded }: { activity: FlowActivity; onUploaded: (path: string) => void }) {
  if (!activity.requires_photo) return null;
  return <div className="col-span-2 grid gap-2 pl-8 md:order-3 md:flex-1 md:pl-0 sm:grid-cols-2"><div><p className="mb-1 text-xs text-muted-foreground">Foto inicial</p><PhotoButton label="Tomar foto inicial" value={activity.initial_photo} onUploaded={onUploaded} /></div><div><p className="mb-1 text-xs text-muted-foreground">Foto final</p><PhotoButton label="Tomar foto final" value={activity.final_photo} onUploaded={(path) => onUploaded(`__FINAL__${path}`)} /></div></div>;
}
...
function AreaFinalPhoto({ area, activities, onUploaded }: { area: ExecutionArea; activities: Execution["activities"]; onUploaded: (path: string) => void }) {
  ...
  const complete = activities.every((activity) => activity.completed || activity.not_applicable);
