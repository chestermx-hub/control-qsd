                ...
                {areaActivities.map((activity, index) => (
                  <div key={activity.id} className={`grid grid-cols-[auto_minmax(0,1fr)] gap-x-3 gap-y-2 px-3 py-3 md:flex md:items-center ${activity.not_applicable ? "bg-muted/50" : ""}`}>
                    ...
                    <div className="col-span-2 flex items-center justify-between gap-3 pl-8 md:contents">
                       <label className={`flex min-h-11 items-center gap-2 text-xs ${activity.requires_photo && !activity.initial_photo ? "cursor-not-allowed text-amber-700" : "text-muted-foreground"}`} title={activity.requires_photo && !activity.initial_photo ? "Toma primero la foto inicial" : undefined}>
                         <input className="h-5 w-5 accent-primary" type="checkbox" checked={activity.not_applicable} disabled={activity.completed || (activity.requires_photo && !activity.initial_photo)} onChange={(event) => updateActivity(activity, { not_applicable: event.target.checked, completed: false })} />
                        No aplica
                      </label>
                       {activity.completed ? <Badge className="bg-emerald-600">Lista</Badge> : !activity.not_applicable && <Button size="sm" className="min-h-10" disabled={activity.requires_photo && !activity.initial_photo} title={activity.requires_photo && !activity.initial_photo ? "Toma primero la foto inicial" : undefined} onClick={() => updateActivity(activity, { completed: true })}>Completar</Button>}
                    </div>
                  </div>
                ))}
              </div>
