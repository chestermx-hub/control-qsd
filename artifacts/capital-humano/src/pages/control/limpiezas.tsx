...
type ExecutionArea = { id: number; area_name: string; initial_photo?: string; final_photo?: string; ready: boolean };
...
function ExecutionPage({ catalogs, reload }: { catalogs: Catalogs; reload: () => void }) {
  ...
  return <div className="space-y-5"><div className="flex flex-wrap justify-between gap-3"><div><h2 className="text-xl font-semibold">{execution.cleaning_type.name}</h2><p className="text-sm text-muted-foreground">{execution.client.name} · {execution.execution_date} · {done}/{execution.activities.length} actividades</p></div><div className="flex gap-2"><Button variant="outline" onClick={report}><Download className="mr-2 h-4 w-4" />Reporte PDF</Button><Button variant="outline" onClick={() => setExecution(null)}>Nueva limpieza</Button></div></div><div className="h-2 rounded-full bg-muted overflow-hidden"><div className="h-full bg-primary transition-all" style={{ width: `${(done / execution.activities.length) * 100}%` }} /></div>{execution.areas.map((area) => { const areaActivities = execution.activities.filter(a => (a.area_name || "Área general") === area.area_name); return <Card key={area.id} className={area.ready ? "border-emerald-500/60" : ""}><CardContent className="p-4 space-y-4"><div className="flex items-center gap-3"><div className={`rounded-full h-9 w-9 flex items-center justify-center ${area.ready ? "bg-emerald-500 text-white" : "bg-muted"}`}>{area.ready ? <Check className="h-4 w-4" /> : <span className="text-sm font-semibold">{areaActivities[0] ? execution.activities.indexOf(areaActivities[0]) + 1 : "·"}</span>}</div><div className="flex-1"><p className="font-semibold">{area.area_name}</p><p className="text-xs text-muted-foreground">{areaActivities.length} actividades · {area.ready ? "Área lista" : "Área pendiente"}</p></div><button type="button" aria-label={`Marcar ${area.area_name} como lista`} disabled={!area.initial_photo || !area.final_photo} onClick={() => updateArea(area, { ready: !area.ready })} className={`relative h-6 w-11 rounded-full transition-colors ${area.ready ? "bg-emerald-500" : "bg-slate-300"} disabled:cursor-not-allowed disabled:opacity-50`}><span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition-transform ${area.ready ? "translate-x-6" : "translate-x-1"}`} /></button></div><div className="grid md:grid-cols-2 gap-3"><div><p className="text-xs font-medium mb-2">Foto inicial del área</p><PhotoButton label="Tomar foto inicial" value={area.initial_photo} onUploaded={path => updateArea(area, { initial_photo: path })} /></div><div><p className="text-xs font-medium mb-2">Foto final del área</p><PhotoButton label="Tomar foto final" value={area.final_photo} onUploaded={path => updateArea(area, { final_photo: path })} /></div></div><div className="rounded-md border divide-y">{areaActivities.map((a, index) => <div key={a.id} className={`flex items-center gap-3 px-3 py-3 ${a.not_applicable ? "bg-muted/50" : ""}`}><span className="text-xs text-muted-foreground w-5">{index + 1}.</span><p className={`flex-1 text-sm ${a.not_applicable ? "line-through text-muted-foreground" : ""}`}>{a.description}</p><label className="flex items-center gap-2 text-xs text-muted-foreground whitespace-nowrap"><input type="checkbox" checked={a.not_applicable} disabled={a.completed} onChange={e => updateActivity(a, { not_applicable: e.target.checked, completed: false })} />No aplica</label>{a.completed ? <Badge className="bg-emerald-600">Lista</Badge> : !a.not_applicable && <Button size="sm" disabled={!area.ready} onClick={() => updateActivity(a, { completed: true })}>Completar</Button>}</div>)}</div></CardContent></Card>; })}</div>;
}
